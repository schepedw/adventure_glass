...
Then(/^I should have no unfound images$/) do
  expect(all('img').map(&:has_no_content?).all?{|bool| !bool } ).to be true #ugly way of testing for no missing images. Better way?
end

When(/^I click (?:on |)(?:the |)["'](.*?)['"](?: button|)$/) do |content|
  click_link(content) if page.has_link?(content)
  page.find(:xpath, "//button[contains(., '#{content}')]").click if page.has_xpath?("//button[contains(., '#{content}')]")
